...
function PrincipalLeaveApproval({ leaves, onRefresh }) {
    ...
    const handleApprove = async (leaveId) => {
        try {
            ...
            toast.error(error.response?.data?.error || 'Failed to approve leave. Please try again.');
        }
    };
    ...
    const handleReject = async (leaveId) => {
        try {
            const token = localStorage.getItem('access_token');
            await axios.post(`/api/principal/leaves/${leaveId}/reject/`, 
                { comment: comments[leaveId] || '' },
                { headers: { Authorization: `Bearer ${token}` } }
            );
            setComments(prev => ({ ...prev, [leaveId]: '' }));
    ...
    return (
        <div className="bg-white p-6 sm:p-8 rounded-lg shadow-xl border border-gray-200 max-w-4xl mx-auto my-8">
            <h2 className="text-3xl font-extrabold text-gray-900 mb-6 text-center">Principal Leave Approval</h2>
            
            <div className="mb-6 p-4 bg-purple-50 border border-purple-200 rounded-lg text-purple-800">
                <h3 className="font-bold text-lg mb-2">Pending Approvals: <span className="text-purple-700">{leaves.length}</span></h3>
                <p className="text-sm">
                    Review and take action on leave requests from both staff and HODs.
                    Staff leaves require HOD approval first, while HOD leaves come directly to you.
                </p>
            </div>
                    ...
                    {leaves.map((leave) => (
                        <div key={leave.id} className="bg-gray-50 p-6 rounded-xl shadow-md border border-gray-200">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
                                <div>
                                    <h4 className="font-bold text-xl text-gray-900">{leave.staff_name}</h4>
                                    <p className="text-sm text-gray-600 mt-1">
                                        {leave.staff_department} • <span className="font-medium text-purple-700">{leave.staff_role}</span>
                                    </p>
                                </div>
                            ...
                            {leave.user.role === 'STAFF' && (
                                <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                                    <span className="text-sm font-bold text-green-800">HOD Approval:</span>
                                    <p className="text-sm text-green-700 mt-1">
                                        Approved by {leave.user.department} HOD on {formatDate(leave.hod_approval_date)}
                                    </p>
                                </div>
                            )}
                            ...
                            <div className="mb-5">
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition duration-200 ease-in-out"
...
export default PrincipalLeaveApproval;
